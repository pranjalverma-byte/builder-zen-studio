        ...
        <div className="card wide" style={{gridColumn:'span 3'}}>
          ...
          <div className="contacts">
            <div className="contacts-form">
              ...
              <button className="btn-login" onClick={add} type="button" disabled={!name || !phone || contacts.length>=3}>Add</button>
            ...
            <ul className="contacts-list">
              {contacts.map(c=> (
                <li key={c.id} className="contact-item">
                  ...
                  <button className="remove" onClick={()=>remove(c.id)} aria-label={`Remove ${c.name}`}>Remove</button>
                </li>
              ))}
              {contacts.length===0 && <li className="muted">No contacts added yet.</li>}
            </ul>
          </div>
        </div>
      </DashboardLayout>
